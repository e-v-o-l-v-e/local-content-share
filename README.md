...
{ inputs, ... }: 
{
  ...
  # the import is needed until the nixpkgs PR is merged
  imports = [
...
| `package` | _see below_ | The package used by nixos for the service |
| `openFirewall` | false | Whether nixos should open the port in the firewall |


#### Default nix package

The default package when using this flake is the one from the flake, which is always up to date, the default one in the nixos modules is the nixpkgs one, which will usually have a 2 to 3 days delay with any release.
